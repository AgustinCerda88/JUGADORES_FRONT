const getJugadores = async (page) => {
  const response = await fetch(
    `https://jugadores-back.vercel.app/jugadores?page=${page}&limit=10`
  );
...
const pintarDatos = async ({ results, info }) => {
  ...
  if (info.page > 1) {
    ...
    myDiv$$.appendChild(botones$$);
    botones$$.appendChild(buttonMenos$$);
  }
...
form$$.addEventListener("submit", async (event) => {
  ...
  try {
    const response = await fetch(
      "https://jugadores-back.vercel.app/jugadores",
      {
        method: "POST",
        headers: {
